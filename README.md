...
├── requirements.txt              # 의존성 라이브러리 (버전 고정: streamlit/numpy/pandas/scipy/networkx/folium/plotly 등)
│
├── core/                         # 전 모듈 공유 유틸
│   ├── data_loader.py            # processed/ 로드 + session_state 배선 (임성엽)
│   ├── distance_util.py          # 다익스트라 이송시간 유틸 (임성엽)
│   ├── map_util.py               # Folium 지도·모듈 가이드 공통 컴포넌트
│   └── viz_util.py               # Plotly 테마 자동 연동 헬퍼 (다크/라이트 대응)
│
├── data/
...
모든 모듈은 `run()`을 `__init__.py`에서 노출합니다.  
M4·M6·M7은 순수 로직을 `compute()`로 추가 노출하여 UI 없이 단독 테스트가 가능합니다.

```python
def run() -> None:
    """Streamlit UI 진입점. app.py가 호출. (전 모듈 공통)"""

def compute(data: dict, params: dict) -> dict:
    """순수 로직. UI 없이 단독 테스트 가능. (M4·M6·M7 노출)"""
```
